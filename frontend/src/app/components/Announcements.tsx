...
//     const handler = async () => {
//       let mounted = true;
//       // const sortByParam = sortMode === "relevant" ? undefined : (sortMode as "recent" | "trending" | "relevant");
//       // const sortByParam = sortMode as "recent" | "trending" | "relevant";
//       const trimmedSearch = searchQuery.trim();
//       let sortByParam: "recent" | "trending" | "relevant" = sortMode;
//       if (sortMode === "relevant" && !trimmedSearch) {
//         sortByParam = "recent";
//       }
//       const items = await listAnnouncements({ sortBy: sortByParam, search: trimmedSearch });
//       if (!mounted) return;
//       setAnnouncements(
//         items.map((announcement) => ({
//           id: announcement.id,
...
export default function Announcements() {
  ...
  useEffect(() => {
    ...
    const fetchAnnouncements = async () => {
      ...
      // Only include `search` when the user has actually typed something.
      // Passing an empty string causes the backend to return 400.
      const params = trimmedSearch
        ? { sortBy: sortByParam, search: trimmedSearch }
        : { sortBy: sortByParam };
      const items = await listAnnouncements(params);
      if (!mounted) return;
      setAnnouncements(
        items.map((announcement) => ({
          id: announcement.id,
    ...
    // For non-search triggers (sortMode change, filter change) fire immediately.
    const debounceTimer = setTimeout(fetchAnnouncements, searchQuery ? 400 : 0);
